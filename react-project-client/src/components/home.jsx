...
class Home extends Component {
  ...
  handleSubmit = (ev) => {
    ...
    fetch('http://localhost:3000/api/v1/users', 
      ...
      }).then(response => response.json()).then(user => this.authenticateUser(user))
  }
  ...
  authenticateUser = (user) => {
    ...
    { method: 'POST',
      headers: { 'Content-Type': 'application/json' }, 
      body: JSON.stringify({ 'email': user.email, 'password': user.password_digest})
    }).then(response => response.json()).then(json => this.assignToken(json.auth_token))
  }
